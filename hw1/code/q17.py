...
def cyclic_pla():
	...
	print last_index
#		print t
	#d.input_vectors[0].extend_vector_by_constant(x0).output()

#cyclic_pla()

def rand_start_pla_skew(d):
	w = vector(5)
	updated = True
	counter = 0
	last_index =0 
	...
	while updated:
		...
		for i in range(len(d.input_vectors)):
			t = seq[i]
			if(sign(d.input_vectors[t].inner_product(w))!= d.labels[t]):
				w = d.input_vectors[t].scale(0.5*d.labels[t]).add(w)
				updated = True
				counter = counter +1
				last_index = t
	return counter

def rand_start_pla_exp_skew():
	x0 = 1
	d = fetch_input("hw1_15_train.dat",x0)
	result = list()
	for i in range(2000):
		result.append(rand_start_pla_skew(d))
	
	plt.hist(result, 100, histtype='stepfilled')
	plt.title("Random Cycle PLA Skewed")
