...
class Cart:
    ...
    def __init__(self,dataset,depth):
        ...
        if(depth<1):
            if(self.split==True):
                self.lchild = Cart(self.D1,depth+1)
                self.rchild = Cart(self.D2,depth+1)
            else:
                self.constant = dataset[0,2]
        else:
            self.constant = self.vote(dataset)
        #print self.best_theta,self.direct,self.dim
    def vote(self,dataset):
        pos =0
        neg =0
        for i in range(len(dataset[:,0])):
            if(np.sign(dataset[i,2])==1):
                pos+=1
            else:
                neg+=1
        if(pos>neg):
    ...
    def predict_labels(self,dataset,depth):
        tmp_list = list()
        if(self.split ==False or depth == 1):
            for i in range(len((dataset[:,0]))):
                tmp_list.append([ dataset[i,3],self.constant])
        else:
            D1,D2 = self.split_data(dataset)
            if(self.lchild is not None):
                tmp_list+=(self.lchild.predict_labels(D1,depth+1))
            if(self.rchild is not None):
                tmp_list+=(self.rchild.predict_labels(D2,depth+1))
        return tmp_list
...
def predict_labels(tree,arg_dataset):
    ret= np.array(tree.predict_labels(arg_dataset,0))
...
c = Cart(TrainBag,0)
c.print_struct(0)
print predict_labels(c,Arg_Train)
